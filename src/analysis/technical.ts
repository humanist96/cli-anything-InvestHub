...
export function calculateTechnicalIndicators(data: OHLCV[]): TechnicalIndicators {
  ...
  const std20 = Math.sqrt(
    closes.slice(-20).reduce((sum, v) => sum + Math.pow(v - sma20Val, 2), 0) / 20
  ) || 1
  ...
  return {
    ...
    bollingerUpper, bollingerMiddle: sma20Val, bollingerLower,
    atr,
