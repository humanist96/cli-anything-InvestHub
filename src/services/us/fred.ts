...
import type { FredSeriesResponse, GlobalMacroIndicator } from "../../types/fred-types.js"
...
async function fetchFredSeries(config: FredSeriesConfig): Promise<GlobalMacroIndicator | null> {
  try {
    ...
    const data: FredSeriesResponse = await res.json() as FredSeriesResponse
...
export async function getGlobalMacroOverview(): Promise<GlobalMacroIndicator[]> {
  ...
  const indicators = results.filter((r): r is GlobalMacroIndicator => r !== null)
