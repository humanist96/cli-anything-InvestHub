...
import type { EcosApiResponse, MacroIndicator } from "../../types/ecos-types.js"

const ECOS_BASE_URL = "https://ecos.bok.or.kr/api/StatisticSearch"

function getEcosApiKey(): string {
  const key = process.env.ECOS_API_KEY
  if (!key) throw new Error("ECOS_API_KEY is not configured")
  return key
}
...
async function fetchEcosSeries(config: EcosSeriesConfig): Promise<MacroIndicator | null> {
  try {
    ...
    const data: EcosApiResponse = await res.json() as EcosApiResponse
