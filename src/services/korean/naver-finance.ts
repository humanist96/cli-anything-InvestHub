import { cache, FIVE_MINUTES, ONE_HOUR } from "../../cache/memory-cache.js"
...
export async function getQuote(ticker: string): Promise<StockQuote | null> {
  ...
  try {
    ...
    const basic = await basicRes.json() as Record<string, string>
    const integration = integrationRes.ok ? await integrationRes.json() as Record<string, unknown> : null
    const totalInfos = (integration?.totalInfos ?? []) as Array<{ code: string; value: string }>
    ...
    const quote: StockQuote = {
      ticker,
      name: (basic as Record<string, string>).stockName ?? ticker,
      price,
      change,
      changePercent,
      volume,
...
export async function getMarketIndices(): Promise<MarketIndex[]> {
  ...
  for (const { code, name } of indices) {
    try {
      const res = await fetch(`${NAVER_STOCK_API}/index/${code}/basic`, { headers: HEADERS })
      if (!res.ok) continue
...
function parseChartResponse(text: string): HistoricalData[] {
  ...
  return rows.map((row) => {
    const match = row.match(/\["(\d{8})",\s*([\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+)/)
