...
import type {
  ...
  InsiderActivity,
} from "../../types/dart-insider-types.js"
...
async function fetchExecutiveStock(corpCode: string): Promise<ExecutiveStockEntry[]> {
  ...
  try {
    ...
    const res = await fetch(`${DART_BASE_URL}/elestock.json?${params.toString()}`)
...
async function fetchMajorShareholderChanges(corpCode: string): Promise<MajorShareholderChange[]> {
  ...
  try {
    ...
    const res = await fetch(`${DART_BASE_URL}/hyslrChgSttus.json?${params.toString()}`)
    const data = await res.json() as Record<string, unknown>
...
function normalizeMajorShareholder(entry: MajorShareholderChange): InsiderActivity {
...
export async function getInsiderActivities(stockCode: string): Promise<InsiderActivity[]> {
  const cacheKey = `dart:insider:${stockCode}`
  const cached = cache.get<InsiderActivity[]>(cacheKey)
  if (cached) return cached

  const corpCode = resolveCorpCode(stockCode)
  if (!corpCode) return []
