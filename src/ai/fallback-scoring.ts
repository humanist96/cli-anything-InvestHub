import { getTechnicalScore, type TechnicalIndicators } from "../analysis/technical.js"
import { getFundamentalScore } from "../analysis/fundamental.js"
import { getRatingFromScore, type AIScore, type Factor } from "./score-schema.js"
...
export function generateFallbackScore(input: FallbackInput): AIScore {
  ...
  const fundScore = fundamentals ? getFundamentalScore(fundamentals) : 5
  const sentimentScore = newsSentiment ? newsSentiment.overallScore : 5
  const riskScore = 5

  const aiScore = Math.round(
    (techScore * 0.3 + fundScore * 0.3 + sentimentScore * 0.2 + riskScore * 0.2) * 10
  ) / 10
  ...
  return {
    ...
    dataSources: dataSources ?? {
      ...
      financials: false, naverNews: false, googleNews: false,
    },
...
function buildFactors(
  ...
  if (technicalIndicators) {
    ...
    if (technicalIndicators.priceVsSma200 > 0) factors.push({ name: "200일 이동평균 위 위치", impact: "positive", strength: 3 })
    else factors.push({ name: "200일 이동평균 아래 위치", impact: "negative", strength: 3 })
  }
  ...
  if (newsSentiment && newsSentiment.articles.length > 0) {
    if (newsSentiment.overallScore >= 7) factors.push({ name: "뉴스 감성 긍정적", impact: "positive", strength: 3 })
    else if (newsSentiment.overallScore <= 3) factors.push({ name: "뉴스 감성 부정적", impact: "negative", strength: 3 })
    else factors.push({ name: "뉴스 감성 중립", impact: "neutral", strength: 2 })
  }

  if (factors.length < 3) factors.push({ name: "데이터 제한으로 부분 분석", impact: "neutral", strength: 2 })
