import type { BacktestResult, Condition, EquityPoint, OHLCVBar, StrategyDefinition, TradeEntry } from "./types.js"
...
function computeSMA(closes: readonly number[], period: number): readonly number[] {
  ...
  for (let i = 0; i < closes.length; i++) {
    if (i < period - 1) { result.push(closes[i]); continue }
    let sum = 0
...
function computeEMA(closes: readonly number[], period: number): readonly number[] {
  ...
  for (let i = 0; i < closes.length; i++) {
    ...
    else result.push(closes[i] * k + result[i - 1] * (1 - k))
  }
  return result
}
...
function computeBollingerBands(closes: readonly number[], period = 20) {
...
function buildIndicators(closes: readonly number[]): IndicatorSeries {
  ...
  for (const p of [5, 10, 20, 50, 100, 200]) maMap.set(p, computeSMA(closes, p))
  const emaMap = new Map<number, readonly number[]>()
  for (const p of [12, 26, 50]) emaMap.set(p, computeEMA(closes, p))

  return { rsi, ma: maMap, ema: emaMap, macd, macdSignal, bbUpper, bbLower }
}

function getIndicatorValue(condition: Condition, indicators: IndicatorSeries, closes: readonly number[], index: number): number {
  const period = condition.params.period ?? 14
  switch (condition.indicator) {
    case "RSI": return indicators.rsi[index]
    case "MA": return indicators.ma.get(period)?.[index] ?? closes[index]
    case "EMA": return indicators.ema.get(period)?.[index] ?? closes[index]
    case "MACD": return indicators.macd[index]
    case "MACD_SIGNAL": return indicators.macdSignal[index]
    case "BB_UPPER": return indicators.bbUpper[index]
    case "BB_LOWER": return indicators.bbLower[index]
    case "PRICE": return closes[index]
    default: return closes[index]
  }
}

function evaluateCondition(condition: Condition, indicators: IndicatorSeries, closes: readonly number[], index: number): boolean {
  const val = getIndicatorValue(condition, indicators, closes, index)
...
function evaluateConditions(conditions: readonly Condition[], indicators: IndicatorSeries, closes: readonly number[], index: number): boolean {
...
export function runBacktest(
  ...
  options: { commissionRate?: number; initialCapital?: number } = {}
): BacktestResult {
  ...
  let cash = initialCapital, shares = 0, entryPrice = 0, peakEquity = initialCapital
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    if (i >= minIndex) {
      ...
      if (!inPosition) {
        if (evaluateConditions(strategy.buyConditions, indicators, closes, i)) {
          const maxShares = Math.floor(cash / (price * (1 + commissionRate)))
          if (maxShares > 0) {
            cash -= maxShares * price * (1 + commissionRate)
            shares = maxShares; entryPrice = price
            trades.push({ date: bars[i].date, type: "BUY", price, shares: maxShares })
          }
        }
      } else {
        const returnPct = ((price - entryPrice) / entryPrice) * 100
        const hitStopLoss = strategy.stopLoss !== undefined && returnPct <= strategy.stopLoss
        const hitTakeProfit = strategy.takeProfit !== undefined && returnPct >= strategy.takeProfit
        const shouldSell = evaluateConditions(strategy.sellConditions, indicators, closes, i)

        if (shouldSell || hitStopLoss || hitTakeProfit) {
          cash += shares * price * (1 - commissionRate)
          trades.push({ date: bars[i].date, type: "SELL", price, shares, returnPct: Math.round(returnPct * 100) / 100 })
    ...
    equityCurve.push({ date: bars[i].date, value: Math.round(currentEquity), price })
  }

  return computeMetrics(trades, equityCurve, initialCapital, bars.length)
}

function computeMetrics(trades: readonly TradeEntry[], equityCurve: readonly EquityPoint[], initialCapital: number, totalBars: number): BacktestResult {
  ...
  const cagr = years > 0 ? Math.round(((finalEquity / initialCapital) ** (1 / years) - 1) * 10000) / 100 : 0
  ...
  const stdReturn = dailyReturns.length > 1
    ? Math.sqrt(dailyReturns.reduce((s, v) => s + (v - avgReturn) ** 2, 0) / (dailyReturns.length - 1)) : 1
  const sharpe = stdReturn > 0 ? Math.round((avgReturn / stdReturn) * Math.sqrt(252) * 100) / 100 : 0

  return { totalReturn, cagr, mdd, sharpe, winRate, totalTrades: sellTrades.length, trades, equityCurve }
}
